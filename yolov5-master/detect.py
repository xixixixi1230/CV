...
def seperator(plate_image):
    ...
    kx = 11
    ky = 3
    binary_image = perprocess.get_binary_image(plate_image, color, kx, ky)
    contours, hierachy = cv.findContours(binary_image, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_NONE)
    copy_plate_image = plate_image.copy()
    cv.drawContours(copy_plate_image, contours, 0, (0, 255, 0), 2)
    ...
    for i in range(len(index_list)):
        copy_plate_image = plate_image.copy()
        cv.drawContours(copy_plate_image, [region[index_list[i]]], 0, (0, 255, 0), 2)
    rotate_image = perprocess.rotate(region[index_list[0]], plate_image)
    binary_image = perprocess.get_binary_image(rotate_image, color, kx, ky)
    contours, hierachy = cv.findContours(binary_image, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_NONE)
    region, index_list, shape_list = perprocess.findcontours(contours)
    ...
    for i in range(len(index_list)):
        image = rotate_image.copy()
        cv.drawContours(image, [region[index_list[i]]], 0, (0, 255, 0), 2)
        x0 = region[index_list[i]][0]
        x1 = region[index_list[i]][1]
        x2 = region[index_list[i]][2]
        x3 = region[index_list[i]][3]
...
if __name__ == "__main__":
    file_path='Train/images/21.jpg'
    opt= parse_opt(file_path)
    main(opt)
    plate_image=cv.imread(save_file_path[0])
    candidate_char_images=seperator(plate_image)
