...
def get_color(image):
    ...
    # res_yellow=cv.bitwise_and(image,image,mask=mask_yellow)
...
#     if color=='蓝色':
#         mask = cv.inRange(hsv_image, lower_blue, higher_blue)
#     if color == '绿色':
...
#参数：res图像
#返回值：二值化图像
def get_binary_image(image,color,kx,ky):
    lower_blue = np.array([100, 43, 46])
    higher_blue = np.array([124, 255, 255])
    lower_green = np.array([10, 30, 116])
    higher_green = np.array([76, 211, 255])
    ...
    # 闭合操作
    element = cv.getStructuringElement(cv.MORPH_RECT, (kx, ky))
...
def char_seperator(bin_image,color):
    ...
    image = offset_region
    image1=offset_region.copy()
    def get_white_black(image,image1):
        sorted_regions = []
        height = image.shape[0]
        width = image.shape[1]
        def get_wb(image):
        ...
        if end_1 != 0 :
            ...
            image1 = image1[offset_Y:, end_1:]
            white, black, white_max, black_max = get_wb(image)
            cv.imshow('11image', image)
            cv.waitKey(0)
        if start_11!=0:
            image = image[:, :start_11]
            image1 = image1[:, :start_11]
            white, black, white_max, black_max = get_wb(image)
            cv.imshow('11image', image)
            cv.waitKey(0)
        if end_2 != 0:
            image = image[offset_Y:, end_2:]
            image1 = image1[offset_Y:, end_2:]
            cv.imshow('image',image)
            cv.waitKey(0)
            if start_2 != 0:
                image = image[offset_Y:, :-start_2]
                image1 = image1[offset_Y:,:-start_2]
            if start_11 != 0 and start_2>start_11:
                image = image[offset_Y:, :start_11]
                image1 = image1[offset_Y:, :start_11]
            white, black, white_max, black_max = get_wb(image)
            cv.imshow('11image', image)
            cv.waitKey(0)
        ...
        if abs(black_max - white_max) < 2:
            ...
            image1 = image1[offset_Y:-3, offset_X:-1]
            cv.imshow('image', image)
            cv.waitKey(0)
            white, black, white_max, black_max = get_wb(image)
        # 黑底白字
            ...
            for m in range(start_ + 1, width - 1):
                ...
                if wid<18:
                    ...
                    if (black[m] >= (segmentation_spacing * black_max-5)):
                        end_ = m
                        break
                if m == width - 2 and len(sorted_regions) < 7:
                    end_ = m
                    break
            return end_
        ...
        while n < width - 1:
            n += 1
            print(white[n],(1 - segmentation_spacing-0.01) * white_max)
        ...
        return sorted_regions
    sorted_regions=get_white_black(image,image1)
    if sorted_regions == [0]:
        print('分割字符Plan B:')
        image = offset_region
    ...
    if sorted_regions==[0]:
        ...
        working_region=image1.copy()
        cv.imshow('working_region',working_region)
        cv.waitKey(0)
        # 对汉字区域进行等值线找区域
        # 经验值：汉子区域占整体的1/8
