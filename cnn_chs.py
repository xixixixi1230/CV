...
TRAIN_DIR = 'D:/WorkPlace/YoLov5/yolov5-master/Project/Project/data_chs/'
TEST_DIR = "D:/WorkPlace/YoLov5/yolov5-master/Project/Project/data_chs/"
...
LABEL_DICT = {
    'chuan':34, 'e':35, 'gan':36, 'gan1':37, 'gui':38, 'gui1':39, 'hei':40, 'hu':41, 'ji':42, 'jin':43,
	'jing':44, 'jl':45, 'liao':46, 'lu':47, 'meng':48, 'min':49, 'ning':50, 'qing':51,	'qiong':52, 'shan':53,
	'su':54, 'sx':55, 'wan':56, 'xiang':57, 'xin':58, 'yu':59, 'yu1':60, 'yue':61, 'yun':62, 'zang':63,
	'zhe':64
}
...
# 指定数据元素的类型为：tf.float32
# 指定shape，行不固定、列为图片的行*图片的宽（相当于展平一张图片）：shape=[None, IMAGE_HEIGHT * IMAGE_WIDTH])
# 指定训练集的特征矩阵
# 具体此处：
# x 是用来容纳训练数据样本集的特征矩阵，
# shape 形状定义为 [None, IMAGE_HEIGHT * IMAGE_WIDTH]。
# 第一维定义为 None 表示不确定，后面会被实际的数值替代。
# 因为一开始并不知道会有多少张图片数据会被输入。
# 或者当采取 mini-batch 的梯度下降策略时，可以自由的设置 batch 的大小。
# 第二个维度定义为 IMAGE_HEIGHT * IMAGE_WIDTH，因为数据集中的图片大小为 IMAGE_HEIGHT * IMAGE_WIDTH。
x = tf.placeholder(tf.float32, shape=[None, IMAGE_HEIGHT * IMAGE_WIDTH])
# 指定训练集的标签矩阵，列为：CLASSIFICATION_COUNT，是因为使用 one-hot 编码
# 具体此处：
# y_ 是用来容纳训练数据样本集的标签。
# shape 定义为 [None, CLASSIFICATION_COUNT] 。
# 第一维为 None 与 x 具有相同理由，
# 第二维为 CLASSIFICATION_COUNT 是因为总共有 CLASSIFICATION_COUNT 种类别。（使用了独热编码）
y_ = tf.placeholder(tf.float32, shape=[None, CLASSIFICATION_COUNT])
# 如：IMAGE_HEIGHT = IMAGE_WIDTH = 28
# 每个样本是 IMAGE_HEIGHT * IMAGE_WIDTH (如：784 = 28*28)个列构成的向量。所以输入的是n*784的矩阵。
# 输入到CNN中需要卷积，需要每个样本都是矩阵。
# reshape表示将一组图像矩阵x重建为新的矩阵，该新矩阵的维数为（-1，28，28，1）
# 其中-1表示由实际情况来定，Numpy会根据剩下的维度计算出数组的另外一个shape属性值。
# 具体此处：
...
# 第一个维度的 -1 表示大小待定，优先满足后面 3 个维度，最后再计算第一个维度的大小。
# 这样的：N x (IMAGE_HEIGHT * IMAGE_WIDTH) / (IMAGE_HEIGHT x IMAGE_WIDTH x 1) 。
# 第二、第三维度实际上表示每张输入图片的大小需改为 IMAGE_HEIGHT x IMAGE_WIDTH。
